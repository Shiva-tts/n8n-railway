...
const task_runner_ws_server_1 = require("./task-runner-ws-server");
let TaskRunnerModule = class TaskRunnerModule {
    ...
    async loadTaskRunnerServer() {
        const { TaskRunnerServer } = await Promise.resolve().then(() => __importStar(require('../task-runners/task-runner-server')));
        this.taskRunnerHttpServer = di_1.Container.get(TaskRunnerServer);
        this.taskRunnerWsServer = di_1.Container.get(task_runner_ws_server_1.TaskRunnerWsServer);
    ...
    async startInternalTaskRunner() {
        ...
        this.taskRunnerWsServer.setDisconnectAnalyzer(di_1.Container.get(InternalTaskRunnerDisconnectAnalyzer));
    }
};
