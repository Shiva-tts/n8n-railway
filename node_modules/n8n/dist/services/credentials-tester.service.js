...
let CredentialsTester = CredentialsTester_1 = class CredentialsTester {
    ...
    async testCredentials(user, credentialType, credentialsDecrypted) {
        ...
        const routingNode = new n8n_core_1.RoutingNode(workflow, node, connectionInputData, runExecutionData ?? null, additionalData, mode);
        let response;
        try {
            response = await routingNode.runNode(inputData, runIndex, nodeTypeCopy, { node, data: {}, source: null }, credentialsDecrypted);
        }
        catch (error) {
